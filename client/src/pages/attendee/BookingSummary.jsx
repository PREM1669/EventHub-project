...
export default function BookingSummary() {
  ...
  const { myHolds, seats, clearMyHolds } = useSeatStore()
  const [step, setStep] = useState('summary')
  const { mutate, isPending, error } = useCreateBooking()
  const selectedSeats = seats.filter((seat) => myHolds.some((hold) => hold.seatId === seat._id))
  ...
  function handlePay() {
    setStep('processing')
    window.setTimeout(() => {
      mutate({ eventId, seatIds: myHolds.map((hold) => hold.seatId) }, {
        onSuccess: (data) => {
          clearMyHolds()
  ...
  return <section className="mx-auto max-w-xl rounded-xl border border-slate-800 bg-slate-900 p-8">
    ...
    <div className="mt-6 space-y-2">{selectedSeats.map((seat) => <div key={seat._id} className="flex items-center justify-between rounded-xl bg-slate-800 p-4"><span>{seat.label} · {seat.tier}</span><strong>{formatPrice(seat.price)}</strong></div>)}<div className="flex items-center justify-between border-t border-slate-700 pt-4 text-lg font-semibold"><span>Total</span><strong>{formatPrice(total)}</strong></div></div>
